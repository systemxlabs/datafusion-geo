...
impl<'a> GeometryScalarTrait for Point<'a> {
    ...
    fn to_geos(&self) -> DFResult<geos::Geometry> {
        let mut coord_seq = geos::CoordSeq::new(1, geos::CoordDimensions::TwoD)?;
        coord_seq.set_x(0, self.x())?;
        coord_seq.set_y(0, self.y())?;

        geos::Geometry::create_point(coord_seq).map_err(|e| {
